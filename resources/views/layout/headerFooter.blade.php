...
<body>
    ...
    <!-- Header Start -->
    <header id="navbar" class="bg-white border-gray-200 fixed w-full top-0 z-50 transition-shadow duration-300">
        <nav>
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div class="hidden w-full md:block md:w-auto" id="navbar-default">
                    <ul
                        ...
                        <li>
                            <a href="{{ route('home') }}"
                                class="block py-2 px-3 {{ request()->routeIs('home') ? 'text-cyan' : 'text-gray-400' }} hover:text-cyan text-xl rounded hover:bg-cyan-100 md:hover:bg-transparent md:border-0 md:p-0"
                                @if (request()->routeIs('home')) aria-current="page" @endif>Home</a>
                        </li>
                        <li>
                            <a href="{{ route('posts') }}"
                                class="block py-2 px-3 {{ request()->routeIs('posts') ? 'text-cyan' : 'text-gray-400' }} hover:text-cyan text-xl rounded hover:bg-cyan-100 md:hover:bg-transparent md:border-0 md:p-0"
                                @if (request()->routeIs('posts')) aria-current="page" @endif>Posts</a>
                        </li>
                        <li>
                            <a href="{{ route('alumni') }}"
                                class="block py-2 px-3 {{ request()->routeIs('alumni') ? 'text-cyan' : 'text-gray-400' }} hover:text-cyan text-xl rounded hover:bg-cyan-100 md:hover:bg-transparent md:border-0 md:p-0"
                                @if (request()->routeIs('alumni')) aria-current="page" @endif>Alumni</a>
                        </li>
                        <li>
                            <a href="{{ route('companies') }}"
                                class="block py-2 px-3 {{ request()->routeIs('companies') ? 'text-cyan' : 'text-gray-400' }} hover:text-cyan text-xl rounded hover:bg-cyan-100 md:hover:bg-transparent md:border-0 md:p-0"
                                @if (request()->routeIs('companies')) aria-current="page" @endif>Companies</a>
                        </li>
                        <li>
                            <a href="{{ route('profile') }}"
                                class="block py-2 px-3 {{ request()->routeIs('profile') ? 'text-cyan' : 'text-gray-400' }} hover:text-cyan text-xl rounded hover:bg-cyan-100 md:hover:bg-transparent md:border-0 md:p-0"
    ...
    <footer class="p-4 bg-cyan md:p-8 lg:p-10">
        <div class="mx-auto max-w-screen-xl text-center">
            ...
            <p class="my-6 text-gray-500">A platform to explore career journeys, alumni profiles, and job opportunities for graduates of the Software Engineering Technology program at Universitas Gadjah Mada. Connect with the community and discover various career paths.
            </p>
